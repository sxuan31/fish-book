...
const SEAFOOD_DATA = [
  ...
  },
  {
    ...
    "baikeImg": "https://bkimg.cdn.bcebos.com/pic/14ce36d3d539b60064f77c68e350352ac75cb283"
  }
];
...
const Badge = ({ children, className }) => (
  <span className={cn("px-2 py-0.5 text-[0.65rem] font-bold tracking-widest uppercase border", className)}>
    {children}
  </span>
);
...
const ImageFallback = ({ src, alt, className }) => {
  ...
  return (
    <div className={cn("relative overflow-hidden bg-[#111] flex items-center justify-center", className)}>
      {!error ? (
        <img
          src={src}
          alt={alt}
    ...
    <div className="relative w-full h-96 lg:h-[32rem] overflow-hidden group bg-black">
      ...
      {images.length > 1 && (
        <>
          <button
            onClick={handlePrev}
            className="absolute left-6 top-1/2 -translate-y-1/2 w-12 h-12 flex items-center justify-center text-white/50 opacity-0 group-hover:opacity-100 hover:text-white transition-all duration-500 disabled:opacity-0"
          >
            <ChevronRight className="w-8 h-8 rotate-180 font-light" strokeWidth={1} />
          </button>
          <button
            onClick={handleNext}
            className="absolute right-6 top-1/2 -translate-y-1/2 w-12 h-12 flex items-center justify-center text-white/50 opacity-0 group-hover:opacity-100 hover:text-white transition-all duration-500"
          >
...
export default function App() {
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden bg-[#0A1017] font-sans text-slate-100">

      {/* 顶部极简导航区域 */}
      <header className="flex-shrink-0 z-40 w-full bg-[#0A1017] hairline-b">
        <div className="px-6 md:px-10 h-20 flex items-center justify-between max-w-[1920px] mx-auto">
          <div className="flex items-center gap-6">
            <div>
              <h1 className="text-2xl font-normal text-white tracking-widest editorial-title">
                OCEANIC <span className="text-amber-500/80 italic">Index</span>
              </h1>
            </div>
          </div>
          ...
          <div className="relative w-64 md:w-96 group">
            <input
              ...
              className="w-full h-11 bg-transparent border-b border-white/20 focus:outline-none focus:border-amber-500/50 text-sm tracking-widest text-slate-200 transition-colors placeholder:text-slate-500 uppercase"
            />
            {searchQuery ? (
              <button
                onClick={() => {
              ...
              <Search className="absolute right-0 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-500" />
            )}
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 flex overflow-hidden max-w-[1920px] mx-auto w-full">
        ...
        {/* === 第一列：左侧目录分类 === */}
        <aside className="w-20 md:w-72 flex-shrink-0 flex flex-col pt-8 hairline-r relative">
          <div className="px-6 mb-12 hidden md:block">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-[0.2em]">Categories</h3>
          </div>
                ...
                {CATEGORIES.map(category => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "w-full flex items-center py-5 md:py-4 px-6 relative transition-colors text-left group",
                        isActive ? "bg-[#131A26]" : "hover:bg-[#131A26]"
                      )}
                    >
                      {isActive && (
                        <motion.div layoutId="navline" className="absolute left-0 top-0 bottom-0 w-1 bg-amber-500/80" />
                      )}
                      <span className={cn(
                        "hidden md:block text-sm uppercase tracking-[0.15em] transition-colors duration-500",
                        isActive ? "text-white font-bold" : "text-slate-400 group-hover:text-slate-200"
                      )}>
                        {category.name}
                      </span>
                      <span className={cn("md:hidden text-xl grayscale opacity-50 transition-all", isActive && "grayscale-0 opacity-100")}>
                        {category.icon}
                      </span>
                    </button>
                  )
        ...
        <section className="w-[340px] md:w-[420px] flex-shrink-0 flex flex-col hairline-r bg-[#0D141D] relative">
          <div className="h-16 flex items-center justify-between px-6 hairline-b">
                  ...
                  {filteredData.map(item => {
                    ...
                    return (
                      <motion.div
                        ...
                        className={cn(
                          "group cursor-pointer flex gap-5 items-center p-5 hairline-b transition-all duration-500",
                          isSelected ? "bg-[#131A26] relative" : "hover:bg-[#131A26]"
                        )}
                      >
                        {isSelected && (
                        ...
                        <ImageFallback
                          ...
                          alt={item.name}
                          className={cn(
                            "w-20 h-24 md:w-24 md:h-28 shrink-0 transition-all duration-500",
                            isSelected ? "opacity-100 brightness-110" : "opacity-75 group-hover:opacity-100"
                          )}
                        />
                        <div className="flex-1 min-w-0 flex flex-col justify-center">
                          <h3 className={cn("text-lg md:text-xl editorial-title truncate transition-colors duration-500", isSelected ? "text-white" : "text-slate-300 group-hover:text-slate-100")}>
                            {item.name}
                          </h3>
                          <p className="text-xs text-slate-400 uppercase tracking-widest truncate mt-1">
                            {item.enName}
                          </p>
                          <div className="flex gap-2 mt-3 block">
                            <span className={cn("text-[10px] uppercase tracking-widest border px-2 py-0.5 whitespace-nowrap", isSelected ? "border-amber-500/40 text-amber-500" : "border-slate-700 text-slate-400")}>
        ...
        {/* === 第三列：右侧极简排版详情 === */}
        <section className="flex-1 bg-[#0A1017] relative overflow-hidden flex flex-col">
          <AnimatePresence mode="wait">
            {selectedSeafood ? (
              <motion.div
              ...
              >
                <ScrollArea.Root className="h-full w-full">
                  <ScrollArea.Viewport className="w-full h-full pb-32">
                    ...
                    {/* 详情正文区 */}
                    <div className="px-10 md:px-14 pt-10 pb-32 max-w-4xl space-y-0">

                      {/* 名称 + 价格标签行 */}
                      <div className="mb-10 pb-8 hairline-b">
                        {/* 品类名称 */}
                        <div className="mb-6">
                          <h2 className="text-3xl md:text-4xl text-white editorial-title mb-1">
                            {selectedSeafood.name}
                          </h2>
                          <p className="text-sm text-slate-400 editorial-title italic tracking-wide font-light">
                        ...
                        <div className="flex flex-wrap gap-2 mb-6">
                          {selectedSeafood.tags.map(tag => (
                            <span key={tag} className="text-[10px] uppercase border border-slate-700 text-slate-400 px-3 py-1 tracking-[0.15em]">
                              {tag}
                            </span>
                          ))}
                        </div>
                        {/* 价格 */}
                        <div>
                          <p className="text-[10px] uppercase tracking-[0.2em] text-slate-500 mb-1">市场参考价</p>
                          <div className="text-xl font-light text-amber-400/90 editorial-title tracking-wider">
                      ...
                      <div className="grid grid-cols-1 gap-0">
                        ...
                        <div className="py-8 hairline-b">
                          <h4 className="flex items-center gap-3 text-[10px] uppercase tracking-[0.22em] text-slate-500 mb-4">
                        ...
                        <div className="py-8 hairline-b">
                          ...
                          <p className="text-2xl editorial-title italic text-slate-100 tracking-wide font-light leading-relaxed">
                        ...
                        {/* 3. 做法推荐 */}
                        <div className="py-8 hairline-b">
                          <h4 className="flex items-center gap-3 text-[10px] uppercase tracking-[0.22em] text-slate-500 mb-5">
                            <span className="w-4 h-px bg-slate-500" />
                            做法推荐
                          </h4>
                          <div className="flex flex-wrap gap-3">
                            {selectedSeafood.cooking.split('、').map((method, i) => (
                              <span
                                key={i}
                                className="px-4 py-2 border border-slate-700 text-slate-300 text-sm tracking-widest uppercase"
                              >
                                {method.trim()}
                              </span>
                            ))}
              ...
              >
                <div className="w-px h-24 bg-gradient-to-b from-transparent via-white/20 to-transparent mb-8" />
                <h3 className="text-4xl text-white/20 editorial-title tracking-widest">OCEANIC</h3>
                <p className="text-xs uppercase tracking-[0.3em] text-slate-500 mt-6">
                  Select an item from the index to explore.
                </p>
              </motion.div>
            )}
          </AnimatePresence>
